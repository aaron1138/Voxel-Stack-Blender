...
# ui_components.py (Modified to add Save/Load Config Buttons)
...
class ImageProcessorThread(QThread):
    ...
    Now supports multi-threading for individual image processing tasks with optimized memory.
    """
    ...
    def _process_single_image_task(
        ...
        current_binary_image_for_task, current_original_image_for_task = core.load_image(current_image_filepath)
        if current_binary_image_for_task is None:
            raise RuntimeError(f"Could not load image data for processing in worker: {current_image_filepath}")
        ...
        output_image_from_core = core.merge_to_output(current_original_image_for_task, receding_gradient)
    ...
    def run(self):
        """The main processing loop, now orchestrating multi-threaded tasks with memory optimization."""
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                # 1. Prime the pump: Fill the worker queue up to max_workers
                for _ in range(self.max_workers):
                    try:
                        ...
                        if current_binary_image_for_cache is None:
                            self.status_update.emit(f"Skipping unloadable image for cache and submission: {filename}")
                            continue 
                        ...
                        self.progress_update.emit(int((submitted_count / total_images) * 100))
                    except StopIteration:
                        break # No more images to submit

                # 2. Main processing loop: Process completed tasks and submit new ones
                while active_futures:
                    if not self._is_running:
                        self.status_update.emit("Processing stopped by user.")
                        break

                    done, pending = concurrent.futures.wait(active_futures, return_when=concurrent.futures.FIRST_COMPLETED, timeout=0.1)
            ...
            for future in active_futures:
                ...
                try:
                    ...
                    import traceback
                    error_detail = f"An image processing task failed during finalization: {exc}\n{traceback.format_exc()}"
                    self.error_signal.emit(error_detail)
                    self._is_running = False
            ...
            import traceback
            error_info = f"An unexpected error occurred in the main processing thread: {e}\n\nTraceback:\n{traceback.format_exc()}"
...
class ImageProcessorApp(QWidget):
    ...
    def init_ui(self):
        ...
        main_processing_layout.addLayout(config_buttons_layout)
    ...
    def load_settings(self):
        ...
        # Load from the global config instance (which itself loads from JSON)
        ...
        # IMPORTANT: Also apply settings to the XY blend tab.
        # This ensures its internal state (like the pipeline list) is re-populated.
    ...
    def save_settings(self):
        ...
        # Update app_config from simple UI elements *before* saving the whole config
        try:
        ...
        try:
            ...
            config.fixed_fade_distance = 10.0

        try:
            config.thread_count = int(self.thread_count_edit.text())
        except ValueError:
            config.thread_count = DEFAULT_NUM_WORKERS
        config.thread_count = max(1, config.thread_count)

        config.input_folder = self.input_folder_edit.text()
        config.output_folder = self.output_folder_edit.text()
        
        # The xy_blend_tab's internal signals automatically update config.xy_blend_pipeline
        # So no explicit call needed here for that.
        
        # Save the entire app_config to its default JSON file
        config.save("app_config.json")

    def _save_config_to_file(self):
        """Opens a file dialog to save the current configuration to a custom JSON file."""
        # First, ensure the current UI state is saved into the global config object
        self.save_settings() 

        filepath, _ = QFileDialog.getSaveFileName(
            self, "Save Configuration File", "custom_config.json", "JSON Files (*.json);;All Files (*)"
        )
        if filepath:
            try:
                config.save(filepath)
                QMessageBox.information(self, "Save Config", f"Configuration saved to:\n{filepath}")
            except Exception as e:
                QMessageBox.critical(self, "Save Config Error", f"Failed to save configuration:\n{e}")

    def _load_config_from_file(self):
        """Opens a file dialog to load a configuration from a custom JSON file."""
        filepath, _ = QFileDialog.getOpenFileName(
            self, "Load Configuration File", "", "JSON Files (*.json);;All Files (*)"
        )
        if filepath:
            try:
                # Load config into a temporary instance to check if successful before replacing global config
                loaded_config = Config.load(filepath) 
                
                # If loading was successful, replace the global config instance's content
                # This ensures the global 'config' object is updated, not just a local variable.
                config.__dict__.clear() # Clear existing content
                config.__dict__.update(loaded_config.__dict__) # Update with loaded content
                
                # Now, re-populate the UI from the newly loaded global config
                self.load_settings() 

                QMessageBox.information(self, "Load Config", f"Configuration loaded from:\n{filepath}")
            except Exception as e:
                QMessageBox.critical(self, "Load Config Error", f"Failed to load configuration:\n{e}")


    def closeEvent(self, event):
        """Saves settings on exit."""
        self.save_settings() # Save current UI state to app_config and default JSON
    ...
    def show_error(self, message):
        # Print the error message to the console for easy copy-paste
        print(f"\n--- PROCESSING ERROR ---")
    ...
    def set_ui_enabled(self, enabled):
        ...
        self.tab_widget.setTabEnabled(self.tab_widget.indexOf(self.xy_blend_tab), enabled)
