...
# pyside_xy_blend_tab.py (Updated)
...
class XYBlendTab(QWidget):
    ...
    This class now acts as a container and manager, delegating LUT editing
    to the specialized LutEditorWidget.
    """
    ...
    def _setup_ui(self):
        ...
        main_layout.addLayout(right_panel_layout, 1) # Give details panel stretch factor
        
        # --- Collapsible Far Right Panel for LUT Table ---
        far_right_panel_layout = QVBoxLayout()
        far_right_panel_layout.setAlignment(Qt.AlignTop)

        self.toggle_table_button = QPushButton("Show LUT Values >")
        self.toggle_table_button.setCheckable(True)
        self.toggle_table_button.setChecked(False) # Default to collapsed
        self.toggle_table_button.setStyleSheet("QPushButton { text-align: left; padding: 4px; max-width: 150px; }")
        far_right_panel_layout.addWidget(self.toggle_table_button)
        ...
        self.lut_table_group.setVisible(False) # Hide the group by default
        far_right_panel_layout.addWidget(self.lut_table_group)

        main_layout.addLayout(far_right_panel_layout, 0) # No stretch
    ...
    def _connect_signals(self):
        ...
        self.toggle_table_button.toggled.connect(self._toggle_lut_table_panel)

    def _toggle_lut_table_panel(self, checked: bool):
        """Shows or hides the LUT table panel."""
        self.lut_table_group.setVisible(checked)
        if checked:
            self.toggle_table_button.setText("Hide LUT Values <")
        else:
            self.toggle_table_button.setText("Show LUT Values >")

    def _on_lut_params_changed(self):
        """Slot to receive updates from the LutEditorWidget."""
    ...
    def _show_params_for_type(self, op_type: str):
        ...
        widget_map = {
            ...
            "apply_lut": self.apply_lut_widget
        }
    ...
    def _update_param_in_config(self, sender_widget, param_name: str, data_type: type, allow_none_if_zero: bool = False):
        ...
        try:
            value_to_set = None
            if allow_none_if_zero and data_type(text) == 0: value_to_set = None
            else: value_to_set = data_type(text.replace(',', '.'))
    ...
    def _reorder_operations_in_config(self, parent, start, end, destination, row):
        # Adjust destination row if moving item downwards in the list
        if row > start:
            row += 1
        moved_op = self.config.xy_blend_pipeline.pop(start)
        self.config.xy_blend_pipeline.insert(row -1, moved_op)
        self._update_operation_list()
        self.ops_list_widget.setCurrentRow(row - 1)
    ...
    def _update_lut_table(self, lut_array: np.ndarray):
        """Populates the QTableWidget with the 256 LUT values."""
