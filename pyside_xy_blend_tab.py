...
class XYBlendTab(QWidget):
    ...
    def _connect_signals(self):
        ...
        self.gaussian_anisotropic_checkbox.toggled.connect(lambda checked: self._update_checkbox_param_in_config(checked, "enable_anisotropic_correction"))
        self.bilateral_anisotropic_checkbox.toggled.connect(lambda checked: self._update_checkbox_param_in_config(checked, "enable_anisotropic_correction"))
        self.median_anisotropic_checkbox.toggled.connect(lambda checked: self._update_checkbox_param_in_config(checked, "enable_anisotropic_correction"))
    ...
    def _populate_params_widgets(self, op: XYBlendOperation):
        ...
        self.median_anisotropic_checkbox.setChecked(op.enable_anisotropic_correction)
    ...
    def _update_checkbox_param_in_config(self, checked: bool, param_name: str):
        current_row = self.ops_list_widget.currentRow()
        if not (0 <= current_row < len(self.config.xy_blend_pipeline)): return

        selected_op = self.config.xy_blend_pipeline[current_row]
        setattr(selected_op, param_name, checked)
