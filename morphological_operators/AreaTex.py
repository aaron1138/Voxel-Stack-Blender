...
def cpp(image):
    ...
    with open('AreaTex.h', 'w') as f:
        f.write("static const unsigned char areaTexBytes[] = {\\n")
        f.write("   ")
        for y in range(image.size[1]):
            for x in range(image.size[0]):
                val = image.getpixel((x, y))

                if n < last: f.write("0x%02x," % val[0])
                else: f.write("0x%02x" % val[0])
                n += 1

                if n < last: f.write(",0x%02x," % val[1])
                else: f.write(",0x%02x" % val[1])
                n += 1
