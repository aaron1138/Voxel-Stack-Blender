# pyside_xy_blend_tab.py (Modified)
...
from PySide6.QtWidgets import (
    ...
    QFileDialog # Added QFileDialog for LUT file operations
)
...
import lut_manager # Import lut_manager for LUT file operations (load/save)

# The LUT_VERSIONS_DIR concept is removed as global LUT management is deprecated
...
class XYBlendTab(QWidget):
    ...
    """
    # No longer emits lut_changed, as LUTs are per-slot and their settings update directly.
    ...
    def _setup_ui(self):
        ...
        self.lut_rodbard_param_edit.setFixedWidth(60)
    ...
    def _on_selected_op_type_changed(self, new_type: str):
        ...
        if current_row >= 0 and current_row < len(self.config.xy_blend_pipeline):
            ...
            # Create a new XYBlendOperation of the new type to get its default parameters
            # Then copy its defaults to the existing selected_op.
            default_new_op = XYBlendOperation(type=new_type)
            
            # Block signals temporarily to prevent redundant updates
            self._block_all_param_signals(True)

            # Copy all relevant attributes from the default new op to the selected op
            for attr_name in [f.name for f in default_new_op.__dataclass_fields__.values()]:
                # Skip 'type' as it's already set
                if attr_name == 'type':
                    continue
                # For nested dataclasses like lut_params, ensure we copy the object itself, not just reference
                if attr_name == 'lut_params' and isinstance(getattr(default_new_op, attr_name), LutParameters):
                    setattr(selected_op, attr_name, LutParameters(**asdict(getattr(default_new_op, attr_name))))
                else:
                    setattr(selected_op, attr_name, getattr(default_new_op, attr_name))
            
            selected_op.type = new_type # Set the type last
    ...
    def _show_params_for_type(self, op_type: str):
        ...
        # Also update the LUT source/gen type stacked widgets if 'apply_lut' is selected
        if op_type == "apply_lut":
            # This is called during _populate_params_widgets to ensure correct sub-stacked widget visibility
            pass
    ...
    def _populate_params_widgets(self, op: XYBlendOperation):
        ...
        if op.type == "gaussian_blur":
            ...
            # This will also trigger _on_lut_source_changed and _on_lut_gen_type_changed via signals
            # which will then set the correct sub-stacked widget for gen/file.
            # We then populate *those* specific fields.
    ...
    def _block_all_param_signals(self, block: bool):
        ...
        widgets_to_block = [
            ...
            self.lut_sqrt_param_edit, self.lut_rodbard_param_edit,
            self.lut_filepath_edit,
            self.lut_load_file_button, self.lut_save_file_button # Buttons should also be blocked during update
        ]
    ...
    def _update_lut_param_in_config(self, param_name: str, text: str, data_type: type, slider: Optional[QSlider] = None, scale_factor: float = 1.0):
        ...
        # Update the parameter directly on the LutParameters object
    ...
    def _load_lut_from_file_for_op(self):
        ...
        if filepath:
            try:
                # Validate the LUT file content (don't load it fully here, just check format)
                # Actually, lut_manager.load_lut already validates, so we can try to load.
        ...
        try:
            if selected_op.lut_params.lut_source == "generated":
                ...
                if selected_op.lut_params.fixed_lut_path and os.path.exists(selected_op.lut_params.fixed_lut_path):
                    ...
                    QMessageBox.warning(self, "LUT File Not Found", "No valid LUT file specified to save, or file does not exist.")
                    return
            else:
    ...
    def apply_settings(self, cfg: Config): # Corrected type hint here
        ...
        self._update_operation_list() 
        self._update_selected_operation_details() # This will populate all fields
