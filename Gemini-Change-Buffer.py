...
def process_image_stack(image_window: List[np.ndarray]) -> np.ndarray:
    ...
    if blend_mode in ["flat", "linear", "cosine", "exp_decay", "gaussian"]:
        ...
        # The user's observation is that the library handles the downcasting to 0-255,
        # so a direct cast to uint8 should be sufficient, with numpy handling the clamping.
    ...
    # Ensure the output of apply_z_lut is explicitly uint8 before proceeding.
    final_image_after_z_lut = lut_manager.apply_z_lut(z_blended_image).astype(np.uint8)
